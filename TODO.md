...
- ❌ **Chưa hoàn thiện: Search API chưa được gọi (đang dùng mock data)**
...
### Ưu tiên cao (Core features chưa có UI)

1. **Upload file/ảnh/video**
   - [ ] Tích hợp nút paperclip vào input
   - [ ] UI chọn file (file picker)
   - [ ] Preview file trước khi gửi
   - [ ] Progress bar upload
   - [ ] Hiển thị file trong message bubble
   - [ ] Download file

2. **Reaction emoji cho message**
   - [ ] Nút "Add reaction" trên message (hover)
   - [ ] Tích hợp EmojiPicker component
   - [ ] Hiển thị reactions dưới message
   - [ ] Click emoji để add/remove reaction
   - [ ] Tooltip hiển thị user đã react

3. **Reply & Thread**
   - [ ] Nút "Reply" trên message (hover/right-click)
   - [ ] UI hiển thị message được reply
   - [ ] UI thread view (side panel/modal)
   - [ ] Badge số lượng replies

4. **Sửa tin nhắn**
   - [ ] Nút "Edit" trên message của mình (hover/right-click)
   - [ ] Input edit message inline
   - [ ] Hiển thị badge "(edited)"
   - [ ] Shortcut: Arrow Up để edit message cuối

5. **Pin message**
   - [ ] Nút "Pin" trên message (hover/right-click)
   - [ ] Section "Pinned messages" ở đầu chat
   - [ ] UI xem tất cả pinned messages
   - [ ] Nút unpin

6. **Vote trong group**
   - [ ] Nút "Create Vote" trong group chat
   - [ ] Modal/form tạo vote
   - [ ] UI vote card trong message
   - [ ] Nút vote và xem kết quả
   - [ ] Progress bar cho mỗi option

7. **Emoji picker trong input**
   - [ ] Nút emoji trong chat input
   - [ ] Tích hợp EmojiPicker component vào input
   - [ ] Insert emoji vào text cursor position

### Ưu tiên trung bình

8. **Seen message (Read receipts)**
   - [ ] Hiển thị tích xanh/xám trên message
   - [ ] Tooltip "Seen by X, Y, Z"
   - [ ] Update realtime qua socket

9. **Upload avatar**
   - [ ] File picker cho avatar
   - [ ] Crop image UI
   - [ ] Upload và preview
   - [ ] Update avatar realtime

10. **Group management**
    - [ ] UI thêm/xóa thành viên
    - [ ] UI promote/demote role
    - [ ] Settings modal cho group
    - [ ] UI rời nhóm

11. **Verify email**
    - [ ] Banner "Please verify your email"
    - [ ] Button gửi lại email verification
    - [ ] Page xác nhận email

### Ưu tiên thấp

12. **AI Summarize**
    - [ ] Nút "AI Summarize" trong conversation
    - [ ] Modal hiển thị summary
...
16. **Last seen time**
    - [ ] Hiển thị "Last seen X minutes ago"
    - [ ] Format time đẹp (just now, 5m ago, etc)

---

## 🎯 Roadmap Phát Triển

### Phase 1: Core Messaging (2-3 tuần)
- Upload file/ảnh
- Reaction emoji
- Sửa tin nhắn
- Seen message
...
### Phase 3: Management & Settings (1-2 tuần)
- Group management UI
- Upload avatar
- Theme & preferences
- Verify email UI

### Phase 4: AI & Polish (1-2 tuần)
- AI Summarize
- OAuth Google
- Notifications
